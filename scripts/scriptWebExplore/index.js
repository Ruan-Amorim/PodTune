...
var ulDeTipos = window.document.getElementById("container-ul-explore");
...
const arrayTypesImage = {
       ...
       "Vibe Anime": "https://i.pinimg.com/originals/0d/ba/9f/0dba9fd5b9bbfe556fe694051584df1c.gif",
       "Openings": "https://i.pinimg.com/originals/65/f6/06/65f606f9844997d368d800c2313c5a5a.gif",
       "Endings": "https://64.media.tumblr.com/3ac2af7c647e5d77eeeb8536d43c4cc5/81628b05b20b9495-88/s640x960/08609a2e75faefce5ef2ba3d41a2cb2ecd24278f.gif",
...
function abrindoTipos(tipo) {
    titulo.innerText = "";
    let numeroDaMusica = 0;
    numberMuisc = 0
    ulDeTipos.innerHTML = '';
    playlistDeMusicas.length = 0;
    var novaLista = ulDeTipos.appendChild(document.createElement("ul"))
    var imageType = novaLista.appendChild(document.createElement("img"))

    backWeb(novaLista);
    geradorImage(tipo,imageType)

    for (let x = 0; x < listaDeMusicas.length; x++) {
        if (tipo == listaDeMusicas[x].tipo) {
            novaLista.appendChild(document.createElement('li')).innerHTML = `<img class="small-music-image" src="${listaDeMusicas[x].capa}"></img><p id="musicaDaPlaylist${numeroDaMusica}"  onclick="clickMusic(${numeroDaMusica})">${listaDeMusicas[x].nome_artista} - ${listaDeMusicas[x].nome_musica}</p>`;
            numeroDaMusica++
            playlistDeMusicas.push(listaDeMusicas[x])
        }
    }
    nomeCantor.textContent = playlistDeMusicas[numberMuisc].nome_artista;
    nomeMusica.textContent = playlistDeMusicas[numberMuisc].nome_musica;
    capaMp3.src = playlistDeMusicas[numberMuisc].capa; 
    audio.src = playlistDeMusicas[numberMuisc].mp3;
    console.log(playlistDeMusicas);
    background.style.backgroundImage = `url(${playlistDeMusicas[numberMuisc].capa})`;
}
//  GERADOR DE IMAGEM DO VOLTAR

function backWeb(local) {
    local.appendChild(document.createElement('p')).innerHTML = `<a href="explore.html">Voltar</a>`;
}

//  GERADOR DE IMAGEM DO TIPO

function geradorImage(tipo,local) {
    local.src = arrayTypesImage[tipo];
}

//  GERADOR DE TIPOS DE MUSICAS
function geradorDeTipos() {
    for (let x = 0; x < arrayTiposDeMusicas.length; x++) {
        ulDeTipos.appendChild(document.createElement("li")).innerHTML = `<p class="lista-tipos-musicas" onclick="abrindoTipos('${arrayTiposDeMusicas[x]}')">${arrayTiposDeMusicas[x]}</p>`;
    }
}
...
function stylePlayer(parametro) { 
    ...
    if (numberStyle == 0) {
        ...
        MP3.style.backgroundImage = `url(${playlistDeMusicas[numberMuisc].capa})`;
